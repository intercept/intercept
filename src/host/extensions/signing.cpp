...
bool intercept::cert::signing::verifyCert(std::wstring_view file_path, types::r_string ca_cert) {
...
intercept::cert::signing::security_class intercept::cert::signing::classify(std::wstring_view file_path) {
    HCERTSTORE hStore = nullptr;
    HCRYPTMSG hMsg = nullptr;
    DWORD dwEncoding, dwContentType, dwFormatType;
    DWORD dwSignerInfo;
    ...
    BOOL fResult = CryptQueryObject(CERT_QUERY_OBJECT_FILE,
        ...
        &hMsg,
        nullptr);
    //This is probably the only real non-signed check. Maybe the next one or two too. the others are just errors
    ...
    unsigned char           derPubKey[2048];
    DWORD         derPubKeyLen = 2048;
    if (!CryptStringToBinaryA("", 0, CRYPT_STRING_BASE64HEADER, derPubKey, &derPubKeyLen, nullptr, nullptr)) { //change string
        if (hStore) CertCloseStore(hStore, 0);
        if (pSignerInfo) LocalFree(pSignerInfo);
        return security_class::not_signed;                                                      //
    }
    ...
    return security_class::not_signed;//verified && status.dwError == 0x0;
}
