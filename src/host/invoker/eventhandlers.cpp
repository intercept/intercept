...
#define EH_EVENT_DEF(x) invoker::get().add_eventhandler(#x, std::bind(&eventhandlers::x, std::placeholders::_1));
    ...
    void eventhandlers::pre_start(game_value_parameter) {
        ...
        get()._ehFunc = sqf_functions::get().registerFunction("InterceptClientEvent"sv, "Forwarder used to call functions in Intercept Plugins"sv, userFunctionWrapper<client_eventhandler>, game_data_type::ANY, game_data_type::ARRAY, game_data_type::ARRAY);
        LOG(INFO, "Pre-start");
        for (auto& module : extensions::get().modules()) {
